...
// (YXZ or XYZ order), and per-axis zoom. Decoration space is the display
// space mirrored across X (game convention), optionally also mirrored
// across Z (flipZ), and shifted by the reconstruction's centering offset.
// Rotations survive the mirror via M·R·M conjugation, M = diag(-1, 1, ±1).
...
export function displayContext(recon, offsetOf) {
  ...
  return { off, flip, order, mirror: new THREE.Matrix4().makeScale(-1, 1, flip) };
}
...
// decoration Euler -> display quaternion (mirror-conjugated)
export function decQuatToDisplay(d, ctx, out = new THREE.Quaternion()) {
  ...
  m.premultiply(ctx.mirror).multiply(ctx.mirror);
